        ...
        private fun initChartDataSetListener() {
            CoroutineScope(Dispatchers.IO).launch {
                chartDataSet.collect { chartDataSet ->
                    ...
                    val data = chartDataSet.data[selectedMetric.value]
                    if (data.isNullOrEmpty()) return@collect
                    chartManager.displayData(data, chartDataSet.referenceTimestamp)
                }
            }
        }
        ...
        private fun initSelectedMetricListener() {
            CoroutineScope(Dispatchers.IO).launch {
                selectedMetric.collect { metric ->
                    val data = chartDataSet.value.data[metric]
                    if (data.isNullOrEmpty()) return@collect
                    chartManager.displayData(data, chartDataSet.value.referenceTimestamp)
                }
            }
        }
